#!/bin/bash
# Integration test for gpu-arbiter idle timeout (vLLM backend, port 11435)
...
# ============================================================================
info "=== Idle timeout test (vLLM on port 11435) ==="
# ============================================================================

SHORT_TIMEOUT=15

info "  Restarting router with IDLE_TIMEOUT=${SHORT_TIMEOUT}s..."
IDLE_TIMEOUT=$SHORT_TIMEOUT $COMPOSE up -d router >/dev/null 2>&1
sleep 2
...
health=$(vllm_curl /health)
running=$(echo "$health" | python3 -c "import sys,json; print(json.load(sys.stdin)['running'])" 2>/dev/null)

if [ "$vllm_status" != "running" ] && [ "$running" = "False" ]; then
    pass "vLLM auto-stopped after idle timeout"
else
    fail "idle timeout" "container=$vllm_status running=$running"
fi
