...
# devai: import paths ported to vLLM v0.25.1 (the per-model image this
# plugin runs under; see deploy/recovery-flags.json). Upstream shipped this
# for an older vLLM layout where protocol types lived in one module and the
# tool-parser ABC under entrypoints.openai; both moved. Logic is byte-for-byte
# NVIDIA's -- only these imports changed.
...
try:  # AnyTokenizer alias removed in newer vLLM; used only in annotations
...
class NemotronToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: AnyTokenizer, tools=None):
        # devai: vLLM v0.25.1's ToolParser ABC __init__ gained a second
        # param `tools: list[Tool] | None`; upstream shipped the single-arg
        # (tokenizer-only) signature, so vLLM's 2-positional instantiation
        # raised "__init__() takes 2 positional arguments but 3 were given".
