...
devai-tools/                  -- Sibling Go module (own go.mod, go 1.25 -- higher than gpu-arbiter's 1.23 because cmd/devai-mcp-modelstatus's MCP go-sdk dependency requires it) hosting first-party devai CLIs. File-per-concern + table-driven-test convention mirrors gpu-arbiter's. Built/tested containerized (`make build-devai-tools`, `make test-devai-tools`), output to devai-tools/bin/ (gitignored), not the repo-root bin/ (the Python devai-agent launcher's own install target).
