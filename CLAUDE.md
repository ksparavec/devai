...
- `docs/gpu-vendors.md` -- reference for the NVIDIA/AMD GPU-vendor overlay (`DEVAI_GPU_VENDOR`, flipped via `make gpu-vendor VENDOR=nvidia|amd` or `devai-agent --gpu-vendor`). Every hardcoded `nvidia.com/gpu=all` call site and why 2 of the 9 are deliberately left NVIDIA-only (plus the gap that the router container is never handed `DEVAI_GPU_DEVICE` in compose, so a router-recreated backend still gets the NVIDIA default), the `devai-lab-gpu` image's ROCm base + PyTorch wheel-index branch, and an honest verification-status note: NVIDIA is fully verified, AMD/ROCm is unverified even at build time (Docker Hub was unreachable in the session this shipped from) pending a real ROCm host.
- `docs/plans/` -- 14 design plans + execution-order README (`docs/plans/README.md` carries the canonical status table; consult it rather than counting files). Highlights: `bench-rewrite` (Phases 1-5 shipped, Phase 6 deferred to live GPU). `sops-age-secrets` (scaffold shipped). `mcp-gateway` (Phases 1+2 shipped). `skypilot-agent-skill` (Phase 1 shipped, Phase 2 plugin install deferred). `gpu-arbiter-cluster-mode` (Phases 1, 1.5, 2 shipped -- the head->worker request path serves for real, though no live two-host serving test has been run; Phase 3 federation optional). `skypilot-fleet-provisioner` (Phase 1 service shipped; Phase 2 Go code exists but is NOT wired -- nothing constructs the client, so cloud burst is inert). Also tracked there: `model-lifecycle-ledger`, `kv-cache-quantization`, `router-shortcircuit`, `router-fanout`, `pi-coding-agent`, `odysseus-borrowed-ideas`, `review-fixes-2026-07`, `card-derived-hints-and-bench-sync`.
...
make cluster-status             # Pretty-print head's /v1/cluster/status (sends the bearer token from DEVAI_HEAD_TOKEN_FILE, default /run/devai/cluster-token; errors out with render instructions when that file is not readable)
...
- `DEVAI_WORKER_HOST` -- worker mode: hostname/IP the head should use to reach this worker (default: the host's own hostname).
- `DEVAI_HEAD_LISTEN_PORT` -- head mode: cluster control plane port (default 11444).
- `DEVAI_HEAD_TOKEN_FILE` -- head mode: path to the bearer token the head validates every control-plane request against (default `/run/devai/cluster-token` -- the same file workers read via `DEVAI_WORKER_TOKEN_FILE`).
...
- `MCP_PORT` -- host port the gateway listens on (default 8088). Published on **127.0.0.1 only**; remote clients need an SSH tunnel or an authenticating reverse proxy.
- `MCP_SECRETS_FILE` -- Phase 2: host path of the rendered tmpfs file (defaults to `/dev/null` so Phase 1 installs boot cleanly). This is the only secrets knob an operator sets; the container-side path is fixed at `/secrets/.env` in the compose `--secrets=` arg.

### SkyPilot fleet-provisioner env vars (opt-in; see docs/skypilot.md)

- `SKYPILOT_API_PORT` -- host port for the API server (default 46580; container side stays at 46580). Published on **127.0.0.1 only** -- the API server ships no authentication and its endpoints spend real cloud money.
- `SKYPILOT_CREDENTIALS_FILE` -- path to sops-rendered cloud creds (defaults to `/dev/null` so the scoped credential mounts alone suffice: `~/.aws`, `~/.config/gcloud`, `~/.config/sky`, each bound read-only; `~/.ssh` and `~/.config/sops` are deliberately NOT mounted).
- `SKYPILOT_API_ENDPOINT` -- reserved for head-mode cloud-burst provisioning. **Not read by gpu-arbiter today**: `skypilot_client.go` / `skypilot_policy.go` exist but `NewSkyPilotClient` has no non-test caller, so setting it currently has no effect and a head routes over the local fleet only.
...
- **devai-skypilot-api-server** (opt-in, profile=`cluster`) -- Long-lived SkyPilot API server (`berkeleyskypilot/skypilot:0.12.1`) on port 46580. Intended to be called by gpu-arbiter in head mode to provision cluster workers across cloud / Slurm / on-prem (that call is not wired yet -- see `SKYPILOT_API_ENDPOINT` above). Published on `127.0.0.1` only: `sky api start --deploy` ships no authentication of its own. Cloud creds come from five narrowly-scoped read-only bind mounts -- `${HOME}/.aws`, `${HOME}/.config/gcloud`, `${HOME}/.config/sky`, `${HOME}/.runpod`, `${HOME}/.lambda_cloud` -- plus the sops-rendered `${SKYPILOT_CREDENTIALS_FILE}` (RunPod / Lambda API keys + `SKYPILOT_API_TOKEN`), which is BOTH sourced via compose `env_file:` (that is what actually delivers the values) and mounted read-only at `/secrets/.env` for on-disk inspection. The whole-`$HOME`-over-`/root` bind that used to supply these is gone: it also handed the container `~/.ssh` and `~/.config/sops/age/keys.txt` -- the age private key behind every `deploy/*.sops.env` -- and those two directories are now deliberately excluded and must never be mounted here. `/root/.sky` is a named volume (rw) holding SkyPilot's own state and self-generated provisioning keypair. See `docs/skypilot.md`.
...
deploy/Dockerfile.worker-bootstrap -- Minimal cloud-VM image SkyPilot launches: arbiter binary + cloud-init + pre-pulled backend images, no JupyterLab/picker/agents (per cluster-mode decision 11). Carries NO probe caches, so a bootstrap worker registers with zero models and 404s every forwarded request until the caches (and podman socket + weights) are mounted -- see docs/worker-bootstrap.md. Build via `make build-worker-bootstrap`.
...
deploy/mcp-secrets.sops.env   -- Encrypted Tier 2 secrets, TRACKED at the canonical name (`.gitignore` un-ignores `deploy/*.sops.env` explicitly; only render targets and `*.env.plain` are ignored). Operators copy `mcp-secrets.sops.env.example` -> fill in real values -> `sops --encrypt` -> commit.
deploy/skypilot-api.env       -- REFERENCE ONLY: documents the non-secret SkyPilot API server knobs (SKYPILOT_API_PORT default 46580). Nothing sources this file either.
deploy/skypilot-credentials.sops.env -- Encrypted RunPod / Lambda API keys + the SKYPILOT_API_TOKEN bearer token for the head <-> API-server channel. Tracked at the canonical name, same as the MCP file above. Only the `.example` templates exist in-tree today.
...
gpu-arbiter/main.go           -- GPU arbiter source (multi-port proxy, ~4100 lines Go). Dispatches on `--mode={single,worker,head}` (default single is byte-identical to pre-cluster behaviour). `main()` is now `runSingleHost(buildArbiter())`, so worker mode can build the identical arbiter.
...
gpu-arbiter/skypilot_policy.go -- SkyPilotPolicy (PickCheapest + per-launch budget gate + BuildLaunchRequest with worker-bootstrap env contract) + IdleTeardownCoordinator (two-step "send shutdown via heartbeat then sky down" sequence; retry-forever-with-exponential-backoff (10s -> 15min cap, which doubles as the log rate limiter), instance-keyed pending entries so a reused cluster name cannot inherit a stuck predecessor, and a conflict guard that refuses `sky down` when a different live worker holds the name -- surfaced via StuckEntries()).
...
devai-tools/internal/routerclient/ -- get_router_status: cluster-head /v1/cluster/status probe, single-mode per-backend /health fallback, unreachable degradation. Both host defaults are the compose service name `devai-router` (the router publishes no host ports). Sends the cluster bearer token (`DEVAI_HEAD_TOKEN_FILE` -> `DEVAI_WORKER_TOKEN_FILE` -> `/run/devai/cluster-token`) on the cluster probe only -- the per-backend `/health` handlers are unauthenticated. A 401 is reported distinctly from unreachable via the `cluster_error` field, so a stale or unmounted token file is diagnosable rather than silently degrading to single/unreachable.
...
docs/plans/                   -- 14 design plans + execution-order README (README.md holds the canonical status table). See "Documentation" section above for the snapshot.
```
