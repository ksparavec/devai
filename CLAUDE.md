...
# Tests
make test-router                # Go unit tests for arbiter (single-host only; 295 test functions (407 incl. subtests) as of 2026-07-29)
make test-devai-tools            # Go unit tests for devai-tools/ (backup, modelcache, routerclient, envfile, gpu-vendor -- 6 packages)
make test-python                # Python stdlib unittests (bench v3, picker, sops/age, MCP, catalog/model-lifecycle, hf-store linking, bench-sync, probe image stamping, agent model-list injection, concurrency bench, hybrid-arch KV-layer counting -- 850 collected on 2026-09-19)
...
make bench-concurrency MODEL=<served-name> [PORT=11436] [LEVELS=1,4,16,32] [MODE=sweep|multiturn]
                                # Concurrency x prefix-reuse. NOT part of `make bench`: it answers a
                                # backend-selection question, not a per-model leaderboard one, and is
                                # GPU-exclusive. `sweep` = concurrency x shared/disjoint prefix;
                                # `multiturn` = sequential growing-history session (the agent-session
                                # shape). See scripts/bench/bench_concurrency.py.
...
- **devai-sglang** -- `lmsysorg/sglang:v0.5.16-cu130` image (pinned; bump via `deploy/backend-flags.yaml` + `make verify-backend-flags`). NVFP4 / safetensors with RadixAttention for multi-turn speedup. Same `sleep infinity` placeholder + on-demand recreate pattern as vLLM. Entrypoint injects `--reasoning-parser` / `--tool-call-parser` from the probe cache. SGLang has no `--enable-auto-tool-choice` analogue -- `--tool-call-parser` alone enables tool parsing. **`--disable-piecewise-cuda-graph` is no longer passed** (dropped on the 2026-07-29 v0.5.10.post1 -> v0.5.16 bump): v0.5.10's piecewise prefill CUDA graph torch.compiled the forward and Dynamo could not trace flashinfer's FP4 JIT path (`modelopt_quant.py:1482`), so every NVFP4 load crashed; v0.5.16 replaced piecewise prefill with the BREAKABLE graph (`cuda_graph_config.prefill.backend='breakable'`), which never takes that path -- measured, Qwen3.5-9B-NVFP4 loads without the flag in 46s with CUDA graphs enabled. The flag still exists on the image, so re-adding it is a one-line change if a future bump regresses. The bump also **rescued Gemma-4-26B-A4B-it-NVFP4** (`unsupported_arch` -> fits 256K, via the first sglang-scoped `deploy/recovery-flags.json` entry: `--moe-runner-backend flashinfer_cutlass`, which the engine's own error names, plus `--mem-fraction-static 0.80` and `expandable_segments`) and **regressed `NVIDIA-Nemotron-Nano-9B-v2-NVFP4`** (v0.5.16's `ReasonerGrammarBackend` refuses to init with `think_end_token '</think>' must encode to exactly one token`, triggered by `--reasoning-parser nemotron_3`; limited to that parser, and the model was already `bench_dropped` on SGLang). See `scripts/model-families.yaml` SGLang NVFP4 status block.
...
scripts/_kv_layers.py         -- `kv_layer_count(text_cfg)`: how many layers actually hold a KV cache. The fit formula used to multiply per-token KV by `num_hidden_layers`, which is wrong for hybrid architectures whose other layers keep constant-size state, not a per-token cache: Qwen3_5ForConditionalGeneration (Qwen3.5/3.6/3.8, Ornith) declares `layer_types` with 3 layers in 4 `linear_attention` -- a 4x overcount (Qwen3.8-27B: 64 layers, 16 with KV) -- and NemotronHForCausalLM declares `hybrid_override_pattern` where only `*` is attention -- a 14x overcount (Nemotron-Nano-9B-v2: 56 layers, 4 with KV). Not cosmetic: `make model-fit FAMILY=qwen3.8` called all 14 rows too large, and an enumerating `--download` then wrote `too_big` ledger verdicts for models that fit, hiding them from the probe -- the only thing that could have proved the estimate wrong. generate-catalog.py records the result as an optional `kv_layers` in each row's `arch` block (omitted when equal to `layers`, so dense rows do not churn); select-models.py `kv_per_token_bytes`, model-picker.py `_hf_kv_gb` and vram-fit.py `Arch` prefer it and fall back to `layers` when absent. Two deliberate limits: sliding-window layers (Gemma 4, gpt-oss) are STILL counted as full -- they do hold a cache, merely a bounded one, so that is a separate and smaller correction -- and any layout that is unrecognised, inconsistent with `num_hidden_layers`, or derives to zero falls back to the old behaviour, because under-counting is the dangerous direction. model-picker.py must NOT import this module (it runs bind-mounted inside older images; see tests/python/test_picker_container_imports.py) -- it only reads the `kv_layers` value from the catalog. Needs a `make catalog-regen` to take effect on an existing models.yaml.
