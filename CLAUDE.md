...
deploy/.bench-cache.json      -- Bench cache (schema v2). Top-level `_meta` block holds `host_env_history` keyed by 12-char SHA-256 id (kernel, driver_version, gpu_name, gpu_memory_gb, cuda_version, captured_at) plus `current_host_env_id` pointer. Every row stamps `host_env_id` so re-benches against a different driver/kernel are auditable. Row keys: `<repo>@<sha>::<backend>` (HF) or `<digest>::<backend>` (Ollama). Rows hold `tasks` (gsm8k/humaneval/tools_use/leak/longctx subsets) + `metrics` (peak/mean VRAM, ttft, tps_sustained_p50, vllm_kv_cache_usage_perc, etc.). Gitignored.
